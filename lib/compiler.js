...
export class Compiler {
  ...
  async parseTheme(zip) {
      // Typically ppt/theme/theme1.xml
      // We'll just look for the first theme file we find or hardcode common path
  ...
  extractThemeColors(themeXml) {
      ...
      // Map keys like 'dk1', 'lt1', 'accent1' to hex values
      const mapColor = (node) => {
  ...
  renderElement(el, relsMap) {
      ...
      // Handle Group Recursion
      if (type === 'grpSp') {
          // Recursive call. Groups have their own coordinate systems (grpSpPr -> xfrm)
          // For simplicity, we'll just render children. 
          // Advanced: Apply group transform.
      ...
      if (type === 'sp') {
          ...
          if (data['p:txBody']) {
              ...
              pList.forEach(p => {
                  let pContent = '';
                  // Paragraph Properties (alignment, etc. - future)
                  ...
                  if (p['a:r']) {
                      ...
                      runs.forEach(r => {
                          let runStyle = '';
                          // Font Size (sz is 1/100pt). Convert to px for layout consistency (96dpi)
                          // 1pt = 1/72 inch. 1px = 1/96 inch.
              ...
              // We append textHtml to content. 
              // Note: The parent div has 'white-space: pre-wrap' in class .slide-element
              // We override it in paragraphs.
              content = textHtml;
          }
      } else if (type === 'pic') {
          tagName = 'img';
